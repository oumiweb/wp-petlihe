const observerOptions$1 = { threshold: 0.1, rootMargin: "0px 0px -20px 0px" };
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        items.forEach((item, i) => {
          setTimeout(() => item.classList.add("is-animated"), i * 800);
        });
...
function animate(element, updateFn, completeFn = () => {
...
const inViewportLines = /* @__PURE__ */ new Set();
...
if (serviceSection) {
  ...
  if (serviceHeading)
    serviceObserver.observe(serviceHeading);
}
...
if (otherLines.length > 0) {
  ...
  otherLines.forEach((line) => {
    if (!(serviceSection == null ? void 0 : serviceSection.contains(line)) && !inViewportLines.has(line)) {
  ...
  cards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      if (img)
        featuredImg.src = img;
      if (breed)
        featuredBreed.textContent = breed;
      if (gender)
        featuredGender.textContent = gender;
      const infoItems = [
        color ? color : "",
